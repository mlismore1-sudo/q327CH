...
ALLOWED_SIC_CODES = [
    "21100", "21200", "26200", "61100", "61200", "61300", "61900",
    "62011", "62012", "62020", "62030", "62090", "63110", "63120",
    "72110", "72190",
]
TARGET_SIC_CODES = {"62012", "72110"}
MANUFACTURING_WHOLESALE_SIC_CODES = set()
ALL_ALLOWED_SIC_CODES = sorted(set(ALLOWED_SIC_CODES))
...
def set_company_checkbox_state(conn: sqlite3.Connection, company_number: str, column: str, checked: bool) -> None:
    allowed = set(DB_CHECKBOX_COLUMNS.values())
...
def upsert_company(conn: sqlite3.Connection, row: Dict[str, Any]) -> None:
    ...
    conn.execute(
        ...
        (
            ...
            row.get("profile_url", ""), preserved[0], int(row.get("target_sic", False)), int(row.get("target_address", False)),
            row.get("target_address_detail", ""), row.get("target_indicators", ""), preserved[1], preserved[2], preserved[3],
        ),
    )
    conn.commit()
...
def process_company(client: CHClient, item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "incorporation_date": item.get("date_of_creation", ""), "company_type": item.get("company_type", ""),
        "international_director": international_director, "international_director_detail": format_flagged_countries(director_details),
        "international_shareholder": international_shareholder, "international_shareholder_detail": format_flagged_countries(shareholder_details),
        "owned_by_company": owned_by_company, "owner_company_name": " | ".join(f"✓ {name}" for name in owner_names),
        "pulled_at": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"), "raw_json": item,
        "profile_url": make_company_profile_url(company_number, company_name), "shortlisted": False,
        "target_sic": target_sic, "target_address": target_address, "target_address_detail": target_address_detail,
...
def build_display_df(db_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in db_df.iterrows():
        ...
        rows.append({
            "Company Name": company_name, "Google Funding Search": make_google_funding_search_url(company_name),
            "Funding secured?": bool(row.get("funding_secured", 0)), "NAB'd": bool(row.get("nabd", 0)),
            "Owned by B/J": bool(row.get("owned_by_bj", 0)), "Shortlist": bool(row.get("shortlisted", 0)),
            "Incorporation Date": row.get("incorporation_date", ""), "Target SIC": "🎯" if bool(row.get("target_sic", 0)) else "",
            ...
            "Target Indicators": row.get("target_indicators", ""), "SIC Code": row.get("sic_code", ""), "Signals": " ".join(signals),
            "International Director": row.get("international_director_detail", ""), "International Shareholder": row.get("international_shareholder_detail", ""),
            "Owned By A Company": row.get("owner_company_name", ""), "Profile": row.get("profile_url", ""),
            "Pulled At": row.get("pulled_at", ""), "company_number": row.get("company_number", ""),
        })
...
def render_sidebar(default_start: date, default_end: date) -> Tuple[date, date, bool, List[str], str, str, bool, bool, bool]:
    ...
    return start_date, end_date, run, selected_signals, sic_search, company_name_search, only_flagged, shortlisted_only, False


def main() -> None:
    apply_custom_css()
    st.title("Companies House New Incorporations Screener")
    st.caption("Pull newly incorporated active companies, screen selected SIC codes, and enrich results with officer and PSC checks.")
    ...
    start_date, end_date, run, selected_signals, sic_search, company_name_search, only_flagged, shortlisted_only, _ = render_sidebar(date.today(), date.today())
    ...
    filtered_df = apply_filters(display_df, only_flagged, selected_signals, sic_search, company_name_search, shortlisted_only, False)
...
- SIC codes pulled from Companies House: `{', '.join(ALL_ALLOWED_SIC_CODES)}`
